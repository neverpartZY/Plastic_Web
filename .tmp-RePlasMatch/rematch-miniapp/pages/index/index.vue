    ...
    <view class="result-overlay" v-if="parseResult" @tap="closeParseResult">
      <view class="result-sheet" @tap.stop>
        <view class="result-header">
          ...
          <view class="result-confidence" :class="'conf-' + parseResult.details.confidenceLabel">
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        const result = await callFunction('getMatches', { page: 1, pageSize: 1 });
        if (result.success) {
    ...
    async loadPrices() {
      try {
        const result = await callFunction('getPrices', {});
        if (result.success) {
          const prices = result.data.prices || [];
          const seen = new Set();
          const snapshot = [];
          for (const p of prices) {
            if (!seen.has(p.category) && snapshot.length < 4) {
              seen.add(p.category);
              snapshot.push(p);
            }
          }
    ...
    async runAiParse(text) {
      ...
      const result = aiParse(text, this.userRole);
      uni.hideLoading();

      if (result.success) {
        this.parseResult = result.result;
        // 保存解析记录
        this.saveParseHistory(result.result);
      } else {
        uni.showModal({
          title: '解析提示',
    ...
    async confirmParseResult() {
      ...
      const data = {
        ...
        location: r.location,
        form: r.form || '',
        specs: r.details.materialTypeLabel || '',
